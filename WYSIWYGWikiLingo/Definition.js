...
rules: [/^(?:(<(.|\n)[^>]*?\/>))/,/^(?:$)/,/^(?:(?=(<\/(.|\n)[^>]*?>)))/,/^(?:(<\/(.|\n)[^>]*?>))/,/^(?:(<(.|\n)[^>]*?>))/,/^(?:(<\/(.|\n)[^>]*?>))/,/^(?:([A-Za-z0-9 .,?;]+))/,/^(?:([ ]))/,/^(?:((\n\r|\r\n|[\n\r])))/,/^(?:(.))/,/^(?:$)/],
conditions: {"htmlElement":{"rules":[0,1,2,4,5,6,7,8,9,10],"inclusive":true},"htmlElementClosing":{"rules":[0,3,4,5,6,7,8,9,10],"inclusive":true},"INITIAL":{"rules":[0,4,5,6,7,8,9,10],"inclusive":true}}
};
return lexer;
})();
parser.lexer = lexer;
