...
lexer.rules = [/^(?:('))/,/^(?:("))/,/^(?:([`]))/,/^(?:.*?(?=(')))/,/^(?:.*?(?=(")))/,/^(?:.*?(?=([`])))/,/^(?:('))/,/^(?:("))/,/^(?:([`]))/,/^(?:([a-zA-Z0-9_-]+)(?=(([=]|[=][>]))))/,/^(?:(([=]|[=][>])))/,/^(?:\s\b)/,/^(?:$)/];
lexer.conditions = {"singleQuoteParameter":{"rules":[0,3,6,7,8,9,10,11,12],"inclusive":true},"doubleQuoteParameter":{"rules":[1,4,6,7,8,9,10,11,12],"inclusive":true},"angleQuoteParameter":{"rules":[2,5,6,7,8,9,10,11,12],"inclusive":true},"INITIAL":{"rules":[6,7,8,9,10,11,12],"inclusive":true}};
return lexer;})()
parser.lexer = lexer;
