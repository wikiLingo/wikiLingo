...
case 6:
        /*php
            WikiLingo_Parameters::add($$[$0-1]->text, $$[$0]->text);
        */
...
rules: [/^(?:('))/,/^(?:("))/,/^(?:(.|\n)*(?=(')))/,/^(?:(.|\n)*(?=(")))/,/^(?:('))/,/^(?:("))/,/^(?:parameterName(?=(([=]|[=][>]))))/,/^(?:(([=]|[=][>]))$)/],
conditions: {"singleQuoteParameter":{"rules":[0,2,4,5,6,7],"inclusive":true},"doubleQuoteParameter":{"rules":[1,3,4,5,6,7],"inclusive":true},"INITIAL":{"rules":[4,5,6,7],"inclusive":true}}
};
return lexer;
})();
parser.lexer = lexer;
